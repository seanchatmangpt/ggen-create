...
from ggen_create.cases import parameterize_body, parameterize_path, render_concrete, values_for
...
from ggen_create.session import add_paths, load_session, set_seed, start_session
...
class ParityTests(unittest.TestCase):
    def test_original_compatible_capture_shape(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertEqual(session["hygen_create_version"], "0.2.0")
            self.assertEqual(session["files_and_dirs"], {"ggen-create.json": True})
    ...
    def test_case_family_matches_original_architecture(self) -> None:
        ...
        rendered, _ = render_concrete(source, "HelloWorld", "CustomerAccount")
        self.assertEqual(
            rendered,
            "CustomerAccount customerAccount customer_account CUSTOMER_ACCOUNT customer-account",
        )

    def test_parameterization_covers_paths_and_contents(self) -> None:
        path, path_replacements = parameterize_path("dist/hello.js", "Hello")
    ...
    def test_add_rejects_outside_root_and_binary(self) -> None:
        with tempfile.TemporaryDirectory() as raw, tempfile.TemporaryDirectory() as outside_raw:
            ...
            session_path = start_session(root, "greeter")
            (outside / "escape.txt").write_text("Hello", encoding="utf-8")
            with self.assertRaisesRegex(GgenCreateError, "PATH_OUTSIDE_CAPTURE_ROOT_REFUSED"):
                add_paths(session_path, [str(outside / "escape.txt")], cwd=root)

            (root / "binary.bin").write_bytes(b"abc\0def")
            with self.assertRaisesRegex(GgenCreateError, "BINARY_FILE_REFUSED"):
                add_paths(session_path, ["binary.bin"], cwd=root)
    ...
    def test_package_is_deterministic_and_versions_changed_generator(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            session_path = start_session(root, "greeter")
            add_paths(session_path, ["package.json", "dist/hello.js"], cwd=root)
            ...
            templates = list((third.package_dir / "templates").glob("*.tmpl"))
            self.assertEqual(len(templates), 3)
            template_text = "\n".join(path.read_text(encoding="utf-8") for path in templates)
            self.assertIn("to: \"dist/{{ row.lower }}.js\"", template_text)

    def test_inspection_is_mechanical(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            (root / "Hello.txt").write_text("Hello hello HELLO", encoding="utf-8")
            session_path = start_session(root, "greeter")
            ...
            self.assertEqual(by_path["Hello.txt"]["target_template"], "{{ row.capitalized }}.txt")
    ...
    def test_full_parity_report_with_bounded_fake_ggen(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            add_paths(session_path, ["package.json", "dist/hello.js"], cwd=source)
            set_seed(session_path, "Hello")
...
for item in meta['files']:
    ...
    body, _ = render_concrete((source_root / item['source']).read_text(), seed, value)
    target = root / rel
...
""",
            ...
            generated_session["files_and_dirs"] = {
                ("dist/hola.js" if key == "dist/hello.js" else key): value
                for key, value in generated_session["files_and_dirs"].items()
            }
            (reference / "ggen-create.json").write_text(
                json.dumps(generated_session, indent=2), encoding="utf-8"
            )
            ...
            self.assertEqual(report["checkpoints"]["P7_PARITY_CROWN"], "ALIVE")
            self.assertEqual(report["checkpoints"]["P6_REVISION_PARITY"], "ALIVE")
