...
from .model import ABOUT, APP_VERSION, GgenCreateError, SESSION_FILE
...
def start_session(root: Path, name: str, filename: str = SESSION_FILE) -> Path:
    root = root.resolve()
    if not root.is_dir():
        raise GgenCreateError("CAPTURE_ROOT_MISSING_REFUSED", f"not a directory: {root}")
    path = root / filename
    if path.exists():
        raise GgenCreateError(
            "SESSION_IN_PROGRESS_REFUSED", f"capture already exists: {path}"
        )
...
def load_session(path: Path) -> dict[str, Any]:
    try:
        ...
        raise GgenCreateError("SESSION_PARSE_REFUSED", f"cannot parse {path}: {exc}") from exc
    ...
    if not isinstance(value, dict) or not required.issubset(value):
        raise GgenCreateError("SESSION_SCHEMA_REFUSED", f"invalid capture schema: {path}")
    if not isinstance(value["files_and_dirs"], dict):
        raise GgenCreateError("SESSION_SCHEMA_REFUSED", "files_and_dirs must be an object")
    return value
...
def _relative_under_root(root: Path, candidate: Path) -> str:
    try:
        ...
        raise GgenCreateError("PATH_MISSING_REFUSED", f"cannot resolve {candidate}: {exc}") from exc
    try:
...
def _candidate_files(path: Path, recursive: bool) -> Iterable[Path]:
    if path.is_symlink():
        raise GgenCreateError("SYMLINK_REFUSED", f"symlinks are not admitted: {path}")
    if path.is_file():
        yield path
        return
    if not path.is_dir():
        raise GgenCreateError("PATH_MISSING_REFUSED", f"path not found: {path}")
    for child in sorted(path.iterdir(), key=lambda item: item.name):
        if child.is_symlink():
            raise GgenCreateError("SYMLINK_REFUSED", f"symlinks are not admitted: {child}")
        if child.is_file():
            yield child
    ...
    for raw in raw_paths:
        ...
        for file_path in _candidate_files(candidate, recursive):
            ...
            if _is_binary(file_path):
                raise GgenCreateError("BINARY_FILE_REFUSED", f"binary file: {rel}")
            try:
                file_path.read_text(encoding="utf-8")
            except UnicodeDecodeError as exc:
                raise GgenCreateError(
                    "NON_UTF8_FILE_REFUSED", f"file is not UTF-8 text: {rel}"
                ) from exc
...
def remove_paths(
    session_path: Path, raw_paths: list[str], *, cwd: Path | None = None
) -> list[str]:
    session = load_session(session_path)
    root = session_path.parent.resolve()
    cwd = (cwd or Path.cwd()).resolve()
...
def set_seed(session_path: Path, value: str) -> None:
    if not value:
        raise GgenCreateError("EMPTY_PARAMETER_REFUSED", "seed must not be empty")
    session = load_session(session_path)
...
def rename_session(session_path: Path, name: str) -> None:
    if not name:
        raise GgenCreateError("EMPTY_GENERATOR_NAME_REFUSED", "name must not be empty")
    session = load_session(session_path)
    session["name"] = name
    save_session(session_path, session)
...
def abort_session(session_path: Path) -> None:
    try:
        ...
        raise GgenCreateError("SESSION_ABORT_REFUSED", str(exc)) from exc
...
def admitted_files(session_path: Path) -> list[str]:
    ...
    for rel, included in session["files_and_dirs"].items():
        ...
        path = root / rel
        if not path.is_file():
            raise GgenCreateError("INCLUDED_PATH_MISSING_REFUSED", f"missing: {rel}")
        result.append(rel)
    if not result:
        raise GgenCreateError("NO_FILES_ADMITTED_REFUSED", "capture has no admitted files")
    return result
